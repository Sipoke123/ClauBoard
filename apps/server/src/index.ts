...
if (adapterMode === "mock") {
  ...
  // Create demo sessions after agents are registered (staggered launches take ~9s for 6 agents)
  // We wait just enough for all agents to register, then create sessions and broadcast snapshot
  if (sessionManager.all().length === 0) {
    setTimeout(() => {
      ...
      const parallelId = `session-${Date.now() + 1}`;
      ...
      // Broadcast snapshot so UI gets sessions immediately
      gateway?.broadcastSnapshot();
    }, 500); // short delay — agents register synchronously in start()
  }
